...
тип самопроверки; смотрите так же: подклассы visitor_*/.py; вообще подобные
...
предопределите метод reset для поддержки множественных, независимых объектов-
...
class FileVisitor:
    ...
    файлов/каталогов переопределяйте метод visitor*; аргумент/атрибут context
    является необязательным и предназанчен для хранения информации,
    спецефической для подкласса; переключатель режима трассировки trace: 0 - нет
    трассировки, 1 - подкаталоги, 2 - добавляются файлы
    """
...
class SearchVisitor(FileVisitor):
    """
    Выполняет поиск строки в файлах, находящихся в каталоге sdtartDir и ниже;
    ...
    определить типы файлов, в которых может выполняться поиск (но могут так же
    ...
    #skipexts = ['.gif', '.jpg', '.pyc', '.o', '.a', '.exe'] # или не допуст. расш.
