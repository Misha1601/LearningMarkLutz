...
sys.path.insert(0, os.getcwd())     # благодоря этому модуль pickle
...
def fetchRecord(db, form):
    try:
        key = form['key'].value
        record = db[key]
        fields = record.__dickt__   # для заполнения строки ответа
...
def updateRecord(db, form):
    if not 'key' in form:
        ...
        if key in db:
            ...
            record = Person(name='?', age = '?')    # evale: строки должны быть
                                                    # заключены в кавычки
