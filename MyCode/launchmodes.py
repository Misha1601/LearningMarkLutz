"""
################################################################################
Запускает команды Python с помощью механизмов командной строки и классаов
...
модуле предпологают, что выполняемый файл "python" находится в системном пути
поиска (смотрите Launcher.py);

можно было бы использовать модуль subprocess, но он сам использует функцию
os.popen(), и к тому же цель этого модуля состоит в том, что бы запустить
независимую программу, а не подключиться к ее потокам ввода-вывода;
можно было бы так же использовать пакет multiprocessing, но данный модуль
предназначен для выполнения программ, а не функций: не имееь смысла запускать
...
def fixWindowsPath(cmdline):
    ...
    return ' '.join([fixedpath] + splitline[1:])    # снова объеденить в строку

class LaunchMode:
    """
    при вызове экземпляра класса выводится метка и запускается команда;
    подклассы формируют строкикоманд для метода run(), если необходимо;
    ...
    def announce(self, text):       # подклассы могут переопределять метод
        print(text)                 # annouce() вместо логики if/elif
...
class Popen(LaunchMode):
    ...
    внмание: может блокировать вызывающую программу, потому что
    канал закрывается немедленно
    """
    def run(self, cmdline):
        cmdline = fixWindowsPath(cmdline)
        os.popen(pypath + ' ' + cmdline)    # предпологается, что нет данных
                                            # для чтения
...
class Start(LaunchMode):
    """
    запускает команду, независимо от вызывающего процесса
...
class StartArgs(LaunchMode):
    """
    только для Windows: в аргументах могут присутствовать
    символы прямого слэша
    """
    def run(self, cmdline):
        assert sys.platform[:3] == 'win'
...
class Spawn(LaunchMode):
    """
    запускает Python в новом процессе, независимым от вызывающего,
    для Windows и Unix; используйте P_NOWAIT для окна dos;
    символы прямого слэша допустимы
    """
    def run(self, cmdline):
