...
выполняет перенаправление stdin, перехватывает sdout, stderr и код завершения,
что бы определить наличие ошибок и отклонений от предыдущих результатов
...
в Python 2.X). Потоки ввода-вывода всегдаинтерпретируются модулем subprocess
как двоичные. Стандартный ввод, аргументы, стандарный вывод и стандартный вывод
ошибок отображаются в файле, находящемся в подкаталогах.
...
Дополнительные возможности расширения: можно было бы реализовать по несколько
наборов аргументов командной строки и/или входных файлов для каждого
тестируемого сценария и запускать их по несколько раз (используя функцию glob
...
for testpath in testfiles:                  # протестировать все сценарии
    ...
    # местоположение файлов для сохранения sdout и stderr,
    ...
    if exitstatus or errdata:       # оба признака ошибки 
        ...
        priorout = open(outpath, 'rb').read() # или сравнить с прежними результ.
        if priorout == outdata:
