"""
От класса Process можно попродить класс, так же, как от класса threading.Thread;
...
class Counter(Process):
    ...
    def run(self):                          # вызываается в новом процессе
...
if __name__ == '__main__':
    ...
    while expected:         # родитель потребляет данные из очереди
        time.sleep(0.5)     # очень напоминает графический интерфейс, хотя в ГИ часто используются потомк
        try:
