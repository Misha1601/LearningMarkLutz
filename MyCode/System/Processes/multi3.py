"""
Реализует взаимодействие с помощью объектов разделяемой памятииз пакета.
...
def showdata(label, val, arr):
    ...
    """
    msg = '%-12s: pid:%4s, glob:%s, value:%s, array:%s'
...
if __name__ == '__main__':
    ...
    # вывести начальное значение в родительском процессе
    ...
    # то же самое, но потомки запускаются паралельно
    ...
    #тоже самое, ног потомки запускаются паралельно
    ...
    # вывендет последние результаты # scalar=12: +6 в родителе, +6 в 6 потомках
