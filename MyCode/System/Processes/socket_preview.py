"""
использует соккеты для обмена между заданиями: запускает потоки
выполнения, взаимодействующие с помощью соккетов; независимые программы таже
могут использовать сокеты для взаимодействия, потому что они принадлежат
системе в целом, как и именованные каналы; смотрите части книги, посвященные
разработке графических интерфейсов и сценариев для и нтернета, где приводятся
более практичные сценарии использования сокетов; некоторым северам может
потребоваться взаимодействие через сокеты с клиентами в виде потоков
...
текст Юникода;
ВНИМАНИЕ: при обращении к функции print в потоках выполненияможет потребоваться
синхранизировать их, если есть вероятность перекрытия по времени;
"""
...
def client(name):
    ...
    sock.close()                            # до 1024 байт в сообщении
