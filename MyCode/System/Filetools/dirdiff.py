"""
################################################################################
Порядок использования: python3 dirdiff.py dir1-path dir2-path
Сравнивает 2 каталога, пытаясь отыскать файлы, присутствующие в одном
и отсутсвующие в другом.
...
их содержимое,  - версию, которая сравнивает результаты вызовов методов .read(),
вы найдете в сценарии diffall.py
################################################################################
"""
...
def reportdiffs(unique1, unique2, dir1, dir2):
    ...
    if not (unique1 or unique2):
        print('Directiry lists are identical')
    else:
        if unique1:
            print('File unique1 to', dir1)
            for file in unique1:
                print('...', file)
        if unique2:
...
def comparedirs(dir1, dir2, files1=None, files2=None):
    ...
    функции listdir может поребоваться передать аргумент типа bytes, если
