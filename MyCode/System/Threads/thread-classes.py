"""
экземпляры класса Tread, сохраняющие информацию о состоянии и обладающие
...
(вместо мьютексов и глобальных переменных), что бы известить главный родительский
...
class Mythread(threading.Thread):       # подкласс класса Thread
    ...
    def run(self):                          # run реализует логику потока
        for i in range(self.count):         # синхронизировать доступ к sdout
            with self.mutex:
